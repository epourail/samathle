...
class Main {
  ...
  static readonly CLIENT_EMAIL: string = <string>(
    process.env.CMS_DIRECTUS_CLIENT_EMAIL
  );
  static readonly CLIENT_PWD: string = <string>(
    process.env.CMS_DIRECTUS_CLIENT_PASSWORD
  );
  static readonly CLIENT_TOKEN: string = <string>(
    process.env.CMS_DIRECTUS_CLIENT_TOKEN
  );
  ...
  static async createUserIfNotExist(client: any) {
    try {
      ...
      if (foundUser) {
        ...
        console.log(
          `[INFO] Client with email: ${Main.CLIENT_EMAIL} not found. Let's create it!`
        );
   ...
   */
  static async createRoleIfNotExist(
    client: any,
    roleName: string,
    userId: string
  ) {
    try {
      ...
      if (foundRole) {
        ...
        console.log(
          `[INFO] Role with name: ${roleName} not found. Let's create it!`
        );
  ...
  static async addRolePermissions(
    ...
    try {
      ...
      const actions: PermissionsAction[] = [
        'create',
        'read',
        'update',
        'delete',
  ...
  static async createUserAndAttachRole(client: any) {
    try {
      ...
      if (createdUser) {
        const role = await Main.createRoleIfNotExist(
          client,
